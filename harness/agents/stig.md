...
Recall with `memory_search` when missing durable context could change your answer or your action — it ranks on what the text actually says. You get cards, not content: `memory_fetch` before relying on anything. Recall is store-wide by default, so check a memory's circle before applying it: this project's memories govern, another project's are analogy at best.
