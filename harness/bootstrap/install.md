...
1. **Upgrade the substrate first, before writing any prompt.** These files describe tool contracts, and an older server answers them differently — a field it does not know is dropped rather than refused, so a Stig written against a newer contract can call a tool, get a success, and have written nothing. Install the current package (`npm i -g @team-monet/monet@latest`, or refresh the zero-install/dev path this machine actually uses), then restart the host so it picks the new server up.
   ...
   **Hand off before the restart if the restart ends this session.** On a host that loads MCP servers only at startup, the restart takes this conversation with it — the fresh session has none of it. Before restarting, give the user a line to paste, naming the source you are running from and the install scope: *"Read &lt;absolute local `bootstrap/install.md` path, or the raw URL this session came from&gt; and resume the UPDATE path at step 1's verification — confirm `memory_checkpoint`'s description describes capturing finds as they happen (or that an unknown key is refused), then continue with steps 2–4 and Phase 7. Host: &lt;host&gt;, scope: &lt;global or this repo&gt;, mode: &lt;team or lead-only&gt;."*
2. Re-read `roster.json`, `agents/*.md`, and `agents/stig.md` from the raw-URL base (or your local `with-monet` checkout).
