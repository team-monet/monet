...
5. **Say the pack changed — do not inventory their store.** Phase 5 is skipped below, but the pack inside it grows, and an entry added after this user installed reaches them by no other route. Tell them in one line — *"the starter pack has changed since you installed; want to look?"* — and stop there. An upgrade is a moment they came to change their tooling, not their norms. If they say yes, run Phase 5's starter-pack section over the whole pack: it already presents one entry at a time and declares only what they approve, so an entry they already hold is one they decline in a word, and their per-entry consent is preserved either way.

   **Do not work out which entries are missing first.** The obvious instrument is `stage_lookup` on each entry's stage, and it is the wrong tool three times over. It is the action-observation surface: every call opens a governed moment and records a rule read for a stage nobody is acting at, leaving conformance questions that can never be honestly answered. It cannot see reach, so an install predating global breadth reads as covered from its own circle while the entry is absent everywhere else. And it cannot tell an entry the user declined from one they never saw, so deciding for them would re-argue a settled call — the thing "their word is final and you record it once" forbids. Asking costs one word and produces no false records.
