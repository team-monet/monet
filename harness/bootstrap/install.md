...
5. **Report new starter-pack entries — do not re-run the pack.** Phase 5 is skipped below, but the pack inside it grows, and an entry added after this user installed reaches them by no other route. Read what the user already has, entry by entry. For rules, the pack names each entry's stage, so `stage_lookup` on that stage returns the live rules there with their bodies. For principles, `memory_overview` lists the ratified skeleton — use it rather than `agent_context`, which omits members a registered standing file already covers, i.e. exactly the setup Phase 5 recommends — but it returns titles, never bodies, so `memory_fetch` each id before comparing, and if its `counts.skeleton` exceeds the list it returned, report the comparison as partial rather than calling the remainder missing. Judge covered-or-not on the body: a user who wrote the same norm in their own words has it, and re-offering it is the duplicate the dumping-ground law forbids. **A local copy does not count as covered.** An install predating global breadth holds the pack bound to a single circle, and read from that circle it is indistinguishable from a global one — no response names a binding's reach. Probe instead: repeat the same `stage_lookup` with `circle` set to any other name. An entry that disappears is local, and is therefore missing everywhere else the user works, which for the new-circle entry means the one moment it exists for. Count those as not covered. The comparison stays small by construction: a handful of rules per stage, not the whole store. Then **report the difference in one line and stop** — *"the starter pack has N entries you don't have; want to see them?"* An upgrade is a moment the user came to change their tooling, not their norms, so spending their attention on a per-entry review they did not ask for is the wrong default. If they say yes, run the pack's own review for those entries only — one at a time, per-entry verdict, declaring just what they approve, exactly as Phase 5 does.
...
and the record should say so. Two arguments apply to the pack. Declare **every** entry
