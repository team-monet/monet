...
5. **Report new starter-pack entries — do not re-run the pack.** Phase 5 is skipped below, but the pack inside it grows, and an entry added after this user installed reaches them by no other route. Read what the user already has, entry by entry: the pack names each rule's stage, so `stage_lookup` on that stage returns the live rules there with their bodies, and `memory_overview` returns the ratified skeleton for the principles — read it there rather than from `agent_context`, which omits skeleton members a registered standing file already covers, i.e. exactly the setup Phase 5 recommends. Compare against the pack as it stands now, and judge covered-or-not on the body — a user who wrote the same norm in their own words has it, and re-offering it is the duplicate the dumping-ground law forbids. The comparison is small by construction: a handful of rules per stage, not the whole store. Then **report the difference in one line and stop** — *"the starter pack has N entries you don't have; want to see them?"* An upgrade is a moment the user came to change their tooling, not their norms, so spending their attention on a per-entry review they did not ask for is the wrong default. If they say yes, run the pack's own review for those entries only — one at a time, per-entry verdict, declaring just what they approve, exactly as Phase 5 does.
