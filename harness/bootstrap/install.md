...
Already set up and just want the latest Stig persona and worker files? You don't need to re-run onboarding — re-run **Phase 4 and Phase 7** against the latest sources, and check the starter pack for entries added since you installed (step 5):
1. **Upgrade the substrate first, before writing any prompt.** These files describe tool contracts, and an older server answers them differently — a field it does not know is dropped rather than refused, so a Stig written against a newer contract can call a tool, get a success, and have written nothing. Install the current package (`npm i -g @team-monet/monet@latest`, or refresh the zero-install/dev path this machine actually uses), then restart the host so it picks the new server up.

   **Then verify the CONTRACT, not the wiring.** `agent_context` answering proves only that some Monet is connected — every version answers it, and the host may resolve a different binary than the shell you upgraded. Check something only the new contract has: read `memory_checkpoint`'s own description in your tool list and confirm it describes capturing finds as they happen rather than recording state at session end. If your host does not expose descriptions to you, probe the behavior instead — call `memory_checkpoint` with one key it cannot know (`{"probe_unknown_key": 1}`): the current contract REFUSES it by name (its input is strict), while an older server accepts the call and silently discards the key. The refusal is the pass. That refusal is also the whole reason this step exists: silent discarding is what turns a capture into a success receipt for a find that was never stored.

   If the verification fails, fix the resolution before going on — find which binary the host actually launches (its MCP config's command/path) and upgrade that one. If the user declines the upgrade, stop here: refreshing the prompts alone is the one combination that fails silently. (Upgrading the package never touches the store or the cached model; only the code.)

   **Hand off before the restart if the restart ends this session.** On a host that loads MCP servers only at startup, the restart takes this conversation with it — the fresh session has none of it. Before restarting, give the user a line to paste, naming the source you are running from and the install scope: *"Read &lt;absolute local `harness/bootstrap/install.md` path, or the raw URL this session came from&gt; and resume the UPDATE path at step 1's verification — confirm `memory_checkpoint`'s description describes capturing finds as they happen (or that an unknown key is refused), then continue with steps 2–5 and Phase 7. Host: &lt;host&gt;, scope: &lt;global or this repo&gt;, mode: &lt;team or lead-only&gt;."*
2. Re-read `roster.json`, `agents/*.md`, and `agents/stig.md` from the raw-URL base (or the `harness/` directory of your local checkout).
3. Re-apply Phase 4's write step **to the same scope the team was installed in** — detect that from where the existing `<!-- BEGIN with-monet:stig -->` / `<!-- with-monet:agent -->` markers already live — in your host's lead-persona file and its worker/subagent locations (the surfaces you found in Phase 1), at whichever scope (user or repo) they were installed. Rewrite the Stig block (between the `<!-- BEGIN with-monet:stig -->` / `<!-- END with-monet:stig -->` markers) and each agent file **in that location** — never silently switch a per-repo install to global. **Reconcile, don't clobber:** preserve your local edits, apply the new changes, guard the invariants, and keep `.bak` (see Phase 4). (Read the `with-monet:mode` marker at the top of the installed Stig block — it records team-vs-lead-only and the installed worker list. No marker (an older install)? Infer from what's installed — worker files present means team, and the refresh then covers the Stig block *and* every worker file; absent means lead-only, and the refresh covers the Stig block only — and ADD the marker while you're rewriting the block. Don't re-ask the team question here; Phase 4's live re-offer covers that.)
4. **Reconcile removals too** (team installs only — when the mode marker says `lead-only`, skip this step: there are no worker files, and the host may have no subagent location to scan): diff the agent `name`s in the roster you just re-read against the installed worker files carrying the `<!-- with-monet:agent -->` marker in the host's subagent location. A marker-carrying file whose agent is no longer in the roster is an orphan from an older roster — left in place, it keeps receiving dispatches for a role the team no longer has. Show the user the orphan list and offer to remove them, backing each up first (same `.bak` discipline as Phase 4). After a removal, update the mode marker's `workers=` list to drop the removed names — the marker must never advertise a worker that isn't installed. Never touch a same-named file *without* the marker — that's the user's own custom subagent, not ours.
5. **Say the pack may have moved on, and say what you cannot check.** Phase 5 is skipped below, but the pack inside it is versioned content — entries get added, and existing ones can need migrating, as both breadth and scope did. Nothing records which version of the pack this install took, so you cannot assert that it changed; say so rather than claiming one. Offer it as a question and stop there: *"the starter pack may have moved on since you installed — I can't tell what you already have, so checking means going through it again. Worth it?"* An upgrade is a moment they came to change their tooling, not their norms, so a no here is a complete answer.

   If they say yes, run Phase 5's starter-pack section over the pack, with one change: **an entry they already hold is a re-declaration question, not a new norm.** The pack's arguments changed — `circle: "*"` and, for rules, `scope: "domain"` — and a copy declared before that is bound to one circle, or to one model, and is absent everywhere else. Re-declaring with the current arguments is what migrates it; declining leaves it exactly as it is. Say which of the two you are asking, every time, or a user protecting a norm they already approved will decline the migration that makes it work. **Run the review from the circle the pack was installed in.** A breadth declaration homes the concept at the caller's own circle, not at the breadth marker (`packages/core/src/engine.ts:13041`), so re-declaring from a different project leaves the original active and adds a second copy homed here — a duplicate, not a migration. If this session is not in that circle, say so and leave the existing entries alone.

   **Do not try to work out first what they are missing.** The obvious instrument is `stage_lookup` on each entry's stage, and it is the wrong tool three times over. It is the action-observation surface: every call opens a governed moment and records a rule read for a stage nobody is acting at, leaving conformance questions that can never be honestly answered. Its rule listing is capped and its bodies clipped, so an equivalent norm of theirs can sit outside what comes back. And it cannot tell an entry the user declined from one they never saw — a decline records nothing — so deciding from that silence would re-argue a settled call, which "their word is final and you record it once" forbids. The cost of asking instead is real and worth naming to them up front: a pack they review again will include entries they already refused. Nothing in the store today can spare them that.

   **Per-repository stores are separate installs.** Where Phase 3 configured a distinct `MONET_STORAGE_DIR` for each repository, this reaches only the store behind the current session — breadth cannot cross that split either. Tell the user the check has to be repeated in each repository they work in.

Skip Phase 1 and Phase 3 (host surfaces and MCP config are already established) and Phase 6 (offer-to-start) and 8 (star). Phase 5 is skipped except for step 5 above — its sort runs on material the user already had, but its starter pack is versioned content that an existing install can fall behind. Phase 2 is NOT skipped — step 1 above is it, in miniature: the package upgrades, the model cache is already warm, so it costs a restart. Then **run Phase 7 (verify)**: the agent files were just rewritten, so confirming workers still launch is most needed here (lead-only installs: verify the refreshed Stig block took effect instead — there are no workers). If your host loads agents/MCP only at startup (most do — you noted this in Phase 1), **reload or restart it** before Phase 7 so the updated Stig + workers take effect; a running session keeps the old prompts until you do. Phase 4 is idempotent, so this is safe to repeat.
...
Register `monet` in your host's MCP config at **user scope** so it's available in every project (template: `harness/mcp/monet.json`). The entry to merge is:
```jsonc
...
- **A fact** is an ordinary `memory_store` — with `sourceRefs` naming the exact line or section it
  came from. That reference is the idempotency key: a sort interrupted before the shrink, and a line
  deliberately left because its readback failed, both leave a rerun needing to know what already
...
then, **for the two principle entries only**, `memory_ratify` with `entrance: "declaration"`,
because their yes is what admitted it and the record should say so. `memory_ratify` takes a
principle or preference and throws on a rule id, so a rule entry is complete at
`memory_declare`; its `declaredBy` is what records the same thing. Two arguments apply to the whole pack. Declare **every** entry
...
`monet materialize add <path> --global`, creating the file if this user never had one. On a
re-run the surface is usually registered already, and `add` refuses a path the manifest holds
rather than treating it as a no-op, so read the manifest first and skip the `add` when the
path is there. Then
