...
function LoginForm() {
  ...
  const performLogin = useCallback(async (tenantSlug: string) => {
    ...
    try {
      // Validate and resolve provider on server before invoking NextAuth.
      ...
      // Set cookie for tenant so the dynamic auth config can resolve the provider.
        ...
        <Card className="w-full max-w-md shadow-lg">
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <div className="space-y-1">
              <p className="font-medium text-lg">Signing in to {tenant}...</p>
              <p className="text-sm text-muted-foreground">Please wait while we redirect you to your SSO provider.</p>
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/40 p-4">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardTitle className="text-2xl font-bold tracking-tight text-primary">Monet</CardTitle>
          <CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-muted-foreground">
                Hint: Use <code className="font-mono text-xs font-bold bg-muted px-1 rounded">test-org</code> for development.
              </p>
            </div>
            ...
            {error && (
              <Alert variant="destructive">
                ...
                <AlertTitle>{isOrgNotFoundError ? "Organization not found" : "Sign in failed"}</AlertTitle>
