...
export function resolveIncoming(input: ResolutionInput): ResolutionDecision {
  ...
  if (obsScore >= tauAttach) {
    ...
    if (centroidScore >= tauAmbiguous) {
      ...
      // the first; nothing had ever asked the second. That is the gate's whole case: which question
      // is being answered, not how any number came out.
      //
      // THE NUMBERS THAT USED TO BE HERE ARE REFUTED (2026-08-26). This branch claimed "the winner's
      // margin over the runner-up puts misfiles' median (0.0335) below correct decisions' p10
      // (0.0346). Under this bar the winner is wrong ~64% of the time." A legality-aware,
      // engine-driven replay on the bge-m3 monet-hq store re-derived both over the LEGAL gap — the
      // quantity the engine gates on — and the separation INVERTS: misfile median 0.0537 sits ABOVE
      // correct p10 0.0481 (n=682), and at tauMargin = 0.12 the winner is wrong 41.7% (n=266), not
      // ~64%. That was one derivation quoted in four places, not four measurements; see
      // embedding.ts's tauMargin block for the full record, the sweep, and the space provenance.
      //
      // Nothing about THIS branch turns on any of it. The gate's own case — tauAttach asks "similar
      // enough", the margin asks "sure it is THIS one" — is a statement about which question is
      // being answered, and holds whatever the distributions do. tauMargin does not move.
      //
      // (An earlier version also opened with "precision of attaches is flat at ~74% across the whole
      // tauAttach range" — RETIRED 2026-08-25, likewise this file's copy of embedding.ts's sentence
      // rather than a second measurement. It was a `Xenova/bge-small-en-v1.5` 384-dim run; on the
      // bge-m3 store precision RISES across the same 0.50 -> 0.75 sweep, 76.3% -> 86.2%.)
      //
