...
export type MomentWriterRole =
  ...
  | "host-hook"
  /** The `monet gate` command, removed with the hook it served. RETAINED FOR READING, as
   *  `host-hook` is, and for the same reason — which is stronger than it first looks. A retired
   *  role is not a value nobody will meet again: THE FOLD CURSOR IS PER-STORE AND THIS SPOOL IS
   *  HOME-LEVEL AND SHARED, so every new project store folds the whole file from byte 0. Drop a
   *  role from this roster and `isWriterRole` rejects its run-start line; the line is consumed, the
   *  cursor advances, and the interception behind it opens a sequence gap at seq 0 — a fabricated
   *  hole in a record that is complete. This was measured, not reasoned: dropping it in 66a47fb
   *  would have manufactured one phantom loss per historical run. Retiring a writer is a decision
   *  about who may WRITE; it is never licence to forget how to READ. */
  | "gate-cli"
