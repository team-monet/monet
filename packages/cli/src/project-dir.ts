 ...
 * anything that needs this resolution — gate-cli.ts's store-less circle derivation, tests — must
 * import it from somewhere that does NOT also run the whole CLI on import. Shared by the `start`
 * action, the `config` command (config-cli.ts pins the project a config was generated for), the
 * source commands, and `gate`'s circle-resolution chain (gate-cli.ts) — one resolution, so a hook
 * and a live session can never disagree about which project they are looking at.
 */
